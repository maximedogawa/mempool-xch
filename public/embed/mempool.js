  ...
  function load() {
    E.rpc("get_blockchain_state")
      .then(function (r) {
        ...
        var ratio = s.mempool_max_total_cost ? Math.min(1, s.mempool_cost / s.mempool_max_total_cost) : 0;
        E.el("count").textContent = (s.mempool_size || 0).toLocaleString("en-US");
        E.el("fill").textContent = E.pct(ratio);
        E.el("cost").textContent = E.cost(s.mempool_cost || 0) + " of " + E.cost(s.mempool_max_total_cost || 0);
        E.el("fees").textContent = E.xch(s.mempool_fees || 0);
        var minFee = s.mempool_min_fees && s.mempool_min_fees.cost_5000000;
        E.el("min").textContent = minFee ? "min " + minFee + " mojo/cost to enter" : "no minimum fee";
        var bar = E.el("bar");
        ...
        E.el("foot").textContent = "Peak #" + s.peak.height.toLocaleString("en-US") + " · " + (s.sync && s.sync.synced ? "synced" : "syncing") + " · refreshes every 15 s";
      })
      .catch(function (e) {
