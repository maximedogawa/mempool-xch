...
(function () {
  ...
  function render(state, records) {
    ...
    for (var i = projected - 1; i >= 0; i -= 1) {
      ...
      d.innerHTML = '<div class="h">next' + (i > 0 ? " +" + i : "") + '</div><div class="s tabular">' + E.pct(fill) + " full</div>" + '<div class="s">in ~' + (eta >= 60 ? Math.round(eta / 60) + " min" : eta + " s") + "</div>";
    ...
    tx.forEach(function (r) {
      var d = document.createElement("div");
      d.className = "block";
      d.innerHTML = '<div class="h tabular">#' + r.height.toLocaleString("en-US") + '</div><div class="s tabular">' + E.xch(r.fees || 0) + ' fees</div><div class="s">' + E.age(r.timestamp * 1000) + "</div>";
      container.appendChild(d);
    });
    E.el("foot").textContent = "Peak #" + state.peak.height.toLocaleString("en-US") + " · " + (state.mempool_size || 0) + " bundles waiting · " + E.cost(mempoolCost) + " cost";
  }
  function load() {
    E.rpc("get_blockchain_state")
      .then(function (r) {
        var s = r.blockchain_state;
        return E.rpc("get_block_records", { start: Math.max(0, s.peak.height - 24), end: s.peak.height + 1 }).then(function (b) {
