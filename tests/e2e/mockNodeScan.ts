...
const GEO: Record<string, unknown> = {
  [SEED_NODES.berlin1]: { ip: SEED_NODES.berlin1, country: "Germany", country_code: "DE", city: "Berlin", latitude: "52.52", longitude: "13.405", organization_name: "Example Telekom" },
  [SEED_NODES.berlin2]: { ip: SEED_NODES.berlin2, country: "Germany", country_code: "DE", city: "Berlin", latitude: "52.55", longitude: "13.38", organization_name: "Example Telekom" },
...
export async function mockNodeScan(page: Page) {
  const answer = (type: "A" | "AAAA") =>
    type === "A"
      ? [SEED_NODES.berlin1, SEED_NODES.berlin2, SEED_NODES.sanFrancisco, SEED_NODES.unplaced].map((data) => ({ name: "dns-introducer.chia.net.", type: 1, TTL: 300, data }))
  ...
  await page.route("https://get.geojs.io/v1/ip/geo.json**", (route) => {
    const ips = (new URL(route.request().url()).searchParams.get("ip") ?? "").split(",").filter(Boolean);
    return json(
      route,
