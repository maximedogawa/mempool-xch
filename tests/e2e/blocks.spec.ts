...
test.describe("block pages", () => {
  ...
  test("block page resolves a pretty URL after a hard load", async ({ page }) => {
    ...
    await expect(
      page.getByText("7bcb5225f8b612363e3e4edfbe0699ed13135570336a23c694c57add8e778cef")
    ).toBeVisible();
    await expect(
      page.getByRole("link", { name: new RegExp(TX_ID.slice(0, 6)) }).first()
    ).toBeVisible();
  });

  test("non-transaction block explains itself and links to transaction blocks", async ({
    page,
  }) => {
    await page.goto("/block/9295513");
    await expect(page.getByRole("heading", { name: /Block 9,295,513/ })).toBeVisible();
    await expect(
      page.getByText(/no spends|carries no transactions|non-transaction/i).first()
    ).toBeVisible();
    ...
    await expect(page.getByTestId("farmed-by")).toContainText(
      "Self-pooling farmer (PlotNFT), rewards claimed to xch1"
    );
  });

  test("transaction block shows parity properties, contents and the coin flow", async ({
    page,
  }) => {
    await page.goto(`/block/${TX_BLOCK_HEIGHT}`);
    ...
    await expect(
      flow
        .getByRole("listitem")
        .filter({ has: page.getByRole("list", { name: "12 coins created" }) })
    ).toBeVisible();
  ...
  test("mempool table sorts by column", async ({ page }) => {
    ...
    await expect(page.getByRole("columnheader", { name: /^age$/i })).toHaveAttribute(
      "aria-sort",
