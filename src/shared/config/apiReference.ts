...
const rpc = (method: string, summary: string, body = "{}"): ApiEndpoint => ({
  method,
  api: "rpc",
  summary,
  body,
});
const indexed = (method: string, summary: string, body: string): ApiEndpoint => ({
  method,
  api: "indexed",
  summary,
  body,
...
export const API_GROUPS: readonly ApiGroup[] = [
  {
    ...
    endpoints: [
      ...
      rpc(
        "get_fee_estimate",
        "Fee-per-cost estimate for one or more target confirmation times.",
        '{ "cost": 6000000, "target_times": [60, 300, 600] }'
      ),
      ...
      rpc(
        ...
        "Pending spend bundles touching a coin.",
        '{ "coin_name": "0x…" }'
      ),
      rpc(
        "get_network_space",
        "Estimated netspace between two header hashes.",
  ...
  },
  {
    ...
    endpoints: [
      ...
      rpc(
        "get_block",
        "The full block (generator info, reward claims) by header hash.",
        '{ "header_hash": "0x…" }'
      ),
      rpc(
        "get_block_spends",
        "Coin spends in a block (custom-node fallback for coin flow).",
        '{ "header_hash": "0x…" }'
      ),
      rpc(
        "get_additions_and_removals",
        "Coins created and removed in a block, for the coin-flow view.",
        '{ "header_hash": "0x…" }'
      ),
      indexed(
        "get_block_transactions",
        "Semantic transaction summaries confirmed in a block, paginated.",
        '{ "height": 9300000, "limit": 50 }'
      ),
    ],
  },
  {
    ...
    endpoints: [
      indexed(
        "get_transaction",
        "One semantic transaction summary by spend-bundle id.",
        '{ "tx_id": "0x…" }'
      ),
      indexed(
        "get_transactions_by_p2",
        "Transaction history for an address's puzzle hash, paginated, newest first.",
        '{ "p2": "0x…", "order": "desc", "limit": 50 }'
      ),
      indexed(
        "get_pending_transactions_by_p2",
        "Pending transactions for an address.",
        '{ "p2": "0x…" }'
      ),
      indexed(
        "get_transactions_by_cat_asset_id",
        "Transaction history for a CAT asset id, paginated.",
      ...
      indexed(
        ...
        "The transaction that created and, if spent, removed a coin.",
        '{ "coin_name": "0x…" }'
      ),
    ],
  },
  {
    ...
    endpoints: [
      ...
      rpc(
        ...
        "Coin records by a list of coin ids.",
        '{ "names": ["0x…"], "include_spent_coins": true }'
      ),
      rpc(
        "get_coin_records_by_puzzle_hash",
        "Coin records paid to a puzzle hash.",
        '{ "puzzle_hash": "0x…", "include_spent_coins": false }'
      ),
      rpc(
        "get_coin_records_by_hint",
        "Coin records by memo hint (how wallets find their own coins).",
        '{ "hint": "0x…", "include_spent_coins": false }'
      ),
      rpc(
        "get_coin_records_by_parent_ids",
        "Coin records created by a list of parent coin ids.",
        '{ "parent_ids": ["0x…"], "include_spent_coins": true }'
      ),
      rpc(
        "get_puzzle_and_solution",
        "The reveal and solution a coin was spent with.",
        '{ "coin_id": "0x…", "height": 9300000 }'
      ),
      rpc(
        "get_memos_by_coin_name",
        "Memo strings attached to a coin's spend.",
        '{ "coin_name": "0x…" }'
      ),
      indexed(
        "get_coin_details",
        "Coin semantics (kind, asset id) when Coinset can classify it.",
        '{ "coin_id": "0x…" }'
      ),
  ...
  },
  {
    ...
    endpoints: [
      indexed(
        ...
        "Confirmed and pending XCH balance for a puzzle hash.",
        '{ "p2": "0x…" }'
      ),
      indexed(
        "get_cat_balances_by_p2",
        "CAT balances for a puzzle hash.",
        '{ "p2": "0x…", "limit": 200 }'
      ),
      indexed("get_nft_balance_by_p2", "NFT count for a puzzle hash.", '{ "p2": "0x…" }'),
      indexed(
        "get_singleton_info",
        "Singleton (NFT/DID) lineage by launcher id.",
        '{ "launcher_id": "0x…" }'
      ),
      indexed(
        "get_latest_nft_coin_by_nft_id",
        "The current unspent coin of an NFT.",
        '{ "nft_id": "nft1…" }'
      ),
    ],
  },
  {
    ...
    endpoints: [
      ...
      indexed(
        "get_offers_by_p2",
        "Offers made from a puzzle hash, one lifecycle status per call.",
        '{ "p2": "0x…", "status": "open", "limit": 25 }'
      ),
      indexed(
        "get_offers_by_cat_asset_id",
        "Offers that offer or request a CAT.",
        '{ "asset_id": "0x…", "status": "open", "filter": "all" }'
      ),
      indexed(
        "get_offers_by_nft_id",
        "Offers that offer or request an NFT.",
        '{ "nft_id": "nft1…", "status": "confirmed" }'
      ),
      indexed(
      ...
      indexed(
        "get_reorgs",
        "Chain reorganisations Coinset detected, newest first.",
        '{ "limit": 20 }'
      ),
      indexed(
