...
import {
  ...
  primaryAsset,
  safeAssets,
} from "@/shared/lib/mempool/assets";
...
export function AssetAmount({
  ...
  assets: CompactAssets | undefined;
  kind?: TxKindHint;
  className?: string;
  full?: boolean;
}) {
  ...
  const tickers = Object.fromEntries(
    assets.cats.map((c) => [c.assetId, tokens.data?.[c.assetId]?.symbol])
  );
  const primary = primaryAsset(assets, kind);
  const all = formatAssets(assets, tickers);
  const text = full
    ? all
    : formatPrimaryAsset(primary, primary.assetId ? tickers[primary.assetId] : undefined);
  return (
