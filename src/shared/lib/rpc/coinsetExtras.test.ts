...
describe("offers", () => {
  test("a confirmed CAT-for-XCH offer keeps both sides and the settlement", () => {
    ...
    expect(o.confirmedTxId).toBe(
      "27c9fe216b056e231d35bf0466920ec102516ce4fe1f01e6868073ab9a0d55d5"
    );
...
describe("clawbacks and reorgs", () => {
  test("clawback coins", () => {
    ...
    expect(list.clawbacks[0]).toEqual({
      coinId: "aa",
      receiverP2: "bb",
      senderP2: "cc",
      seconds: 86400,
  ...
  test("reorg events newest first with depth", () => {
    ...
    expect(list.reorgs[0]).toMatchObject({
...
describe("client", () => {
  const fetchImpl: FetchLike = async (input, init) => {
    ...
    if (url.endsWith("/get_offer")) {
      if (body.offer_id === "00")
        return new Response(
          JSON.stringify({ success: false, error: "Key not found: offer_state/00" })
        );
      return new Response(JSON.stringify(offerState));
    }
    if (url.endsWith("/get_offers_by_p2"))
      return new Response(JSON.stringify({ ...offersByCat, p2: body.p2, status: body.status }));
  ...
  test("offer lookups strip 0x, default to descending order and surface not-found", async () => {
    const offer = await client.getOffer(
      "0xE86A565172a26530728edf9712a34cfb103e1f1395710a6c181c495b4e2ccca5".toLowerCase()
    );
    expect(offer.status).toBe("confirmed");
    const list = await client.getOffersByP2(
      "bacd0883be47be1d6b8b0237521f4fade83683911aeacd63633df615b4a984db",
      "confirmed"
    );
    expect(list.offers).toHaveLength(2);
    await expect(client.getOffer("00")).rejects.toMatchObject({
      kind: "not_found",
