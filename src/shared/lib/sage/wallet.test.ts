...
const ref = (over: Partial<WalletCoinRef>): WalletCoinRef => ({
  coinId: "c",
  amount: 1n,
  address: "xch1a",
  assetKind: "xch",
...
const tx = (spent: WalletCoinRef[], created: WalletCoinRef[]): WalletTx => ({
  id: null,
  height: 1,
  timestamp: 1,
  fee: null,
      ...
      tx(
        [
          ref({
            assetKind: "cat",
            assetId: cat,
            assetName: "Spacebucks",
            ticker: "SBX",
            precision: 3,
          }),
        ],
        [ref({ assetKind: "cat", assetId: cat })]
      ),
