...
function block(poolPuzzleHash: string, height: number, farmerPuzzleHash = poolPuzzleHash): BlockRecord {
  return {
    height,
...
describe("groupPoolShare", () => {
  test("without claims every payout address is its own row, most blocks first with a stable tiebreak", () => {
    const records = [block(PLOT_NFT_B, 1), block(PLOT_NFT_A, 2), block(PLOT_NFT_C, 3), block(PLOT_NFT_C, 4)];
    const result = groupPoolShare(records, NO_CLAIMS);
    expect(result.totalBlocks).toBe(4);
    expect(result.payoutCount).toBe(3);
    expect(result.groups.map((g) => g.key)).toEqual([`address:${PLOT_NFT_C}`, `address:${PLOT_NFT_A}`, `address:${PLOT_NFT_B}`]);
  ...
  test("farmers whose rewards are claimed to the same target merge into one pool", () => {
    const records = [block(PLOT_NFT_A, 1, FARMER), block(PLOT_NFT_B, 2, FARMER), block(PLOT_NFT_B, 3, FARMER), block(PLOT_NFT_C, 4, FARMER)];
    ...
    expect(pool).toMatchObject({ kind: "claim", claimTarget: UNNAMED_TARGET, blocks: 3, share: 0.75, selfPooled: false });
    expect(pool.payouts.map((p) => [p.payoutHash, p.blocks])).toEqual([[PLOT_NFT_B, 2], [PLOT_NFT_A, 1]]);
  ...
  test("a registry pool collects its claim target's farmers and its fixed payout address in one row", () => {
    const records = [block(PLOT_NFT_A, 1, FARMER), block(PLOT_NFT_B, 2, FARMER), block(SPACEFARMERS_PAYOUT, 3), block(NOSSD_PAYOUT, 4)];
    const claims = new Map([
  ...
  test("a group is self-pooled only when every one of its PlotNFTs was", () => {
    const records = [block(PLOT_NFT_A, 1, FARMER), block(PLOT_NFT_B, 2, FARMER), block(PLOT_NFT_C, 3, FARMER)];
    ...
    const mixed = groupPoolShare(records, new Map([[PLOT_NFT_A, claim(UNNAMED_TARGET, true)], [PLOT_NFT_B, claim(UNNAMED_TARGET, false)]]));
  ...
  test("both shares is only flagged when every block of the address pays farmer and pool alike", () => {
    const result = groupPoolShare([block(SOLO, 1), block(SOLO, 2), block(PLOT_NFT_A, 3), block(PLOT_NFT_A, 4, FARMER)], NO_CLAIMS);
