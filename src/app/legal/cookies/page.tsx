...
const NECESSARY = [
  {
    key: "mempool-xch:settings:v1",
    purpose:
      "Network, node address, theme, number of recent blocks, sounds, whether browser notifications are on",
    lifetime: "Until you reset or clear it",
  },
  {
    key: "mempool-xch:tokens:v2",
    purpose: "Cached list of CAT names and icons, so it is not downloaded on every visit",
    lifetime: "Refreshed after 24 hours",
  },
  {
    key: "mempool-xch:history:v1:<network>",
...
export default function CookiesPage() {
  return (
    <LegalPage
      ...
      intro={
        <p>
          {SITE_NAME} sets no cookies. It keeps a few entries in your browser&apos;s local storage,
          which the rules for cookies (Art. 5(3) ePrivacy Directive, § 25 TDDDG) cover in the same
          way. This page lists them.
        </p>
      }
    >
      <Section title="Categories" id="categories">
        <List>
          <li>
            <strong className="text-fg">Strictly necessary</strong>: always on. Needed for features
            you use, and exempt from consent (§ 25(2) no. 2 TDDDG).
          </li>
          <li>
            <strong className="text-fg">Analytics</strong>: usage statistics. Not used at the
            moment; would only run with your consent.
          </li>
          <li>
            <strong className="text-fg">Advertising</strong>: ads and ad measurement. Not used at
            the moment; would only run with your consent.
          </li>
        </List>
      </Section>
      ...
      <Section title="Strictly necessary storage" id="necessary">
        <div
          ...
          tabIndex={0}
          role="region"
          aria-label="Strictly necessary storage"
        >
      ...
      <Section title="Your choice" id="choice">
        <p>
          On your first visit a panel asks for your choice: Reject all, Save my choice or Accept
          all. Your choice is kept for 12 months and then asked again. You can change or withdraw it
          at any time with Cookie settings in the footer or here:
        </p>
        <div>
          <CookieSettingsButton />
        </div>
        <p>
          If your browser sends a Do Not Track or Global Privacy Control signal, analytics and
          advertising stay off and you are not asked.
        </p>
      </Section>

      <Section title="Content from other services" id="third-parties">
        <p>
          Icons, NFT images and some chain data load directly from other services, which receive
          your IP address as any web server does. See section 4 of the{" "}
          <Link href={routes.legalPrivacy()} className="text-accent hover:underline">
            privacy policy
          </Link>
          . x.com/MaximEdogawa on x.com. Since there are no accounts, server logs can only be
