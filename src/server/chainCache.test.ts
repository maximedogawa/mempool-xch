...
function setup(peak: number) {
  ...
  const cache = new ChainCache("mainnet", {
    ...
    hub: { on: (l) => (listeners.add(l), () => listeners.delete(l)), status: () => ({ network: "mainnet", channel: "websocket", connectedAt: 1, lastEventAt: 1, reconnects: 0, counters: {}, seq: 1 }), blockStats },
    now: () => now,
    timers: false,
  });
