...
export function BlockTreemap({
  ...
  const cells = useMemo(
    () =>
      squarify(
        transactions.map((tx) => ({ item: tx, weight: tx.cost })),
        W,
        H
      ),
    [transactions]
  ...
  return (
    <div className="relative">
      <svg
        ...
        role="group"
        aria-label={label}
        className="block h-auto max-h-[320px] w-full rounded-sm"
      >
        <title>{label}</title>
        {cells.map((cell) => {
          const tx = cell.item;
      ...
      {hover ? (
        <div
          role="status"
          className="pointer-events-none absolute left-2 top-2 rounded-sm border border-border bg-bg-elevated px-2.5 py-2 text-xs shadow-card"
        >
