      ...
      {items.map((it) => (
        <button
          ...
          className={cn(
            ...
            tab === it.id ? "bg-surface-2 text-fg" : "text-fg-muted hover:text-fg"
          )}
        >
          {it.label}
      ...
      <div className="flex flex-col items-end">
        ...
        {b?.coins ? (
          <span className="tabular text-[11px] text-fg-faint">
            {formatNumber(b.coins)} coin{b.coins === 1 ? "" : "s"}
          </span>
        ) : null}
      </div>
    </div>
    ...
    <Card>
      <CardHeader
        ...
        action={
          loaded < total ? (
            <span className="text-xs text-fg-faint">
              seen in the first {formatNumber(loaded)} of {formatNumber(total)} transactions ·
              scroll Transactions to find more
            </span>
          ) : null
        }
      />
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <p className="text-xs text-fg-faint">
        Balances, assets, transactions and coins come from your Sage wallet; the mempool, blocks and
        other addresses still come from {networkConfig.label} via the configured node.
        {address ? (
          <>
            {" "}
      ...
      {overview.isLoading && !w ? (
        ...
        <EmptyState
          tone="danger"
          title="Sage did not answer"
          description="The wallet bridge could not be reached. Reopen the app from Sage's app list."
        />
      ) : (
        <>
          <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
            ...
            <StatTile
              ...
              sub={`transactions · ${coinTotal ? formatNumber(coinTotal) : "…"} coins`}
            />
          </div>
          {address ? (
            <Card>
          ...
          <Card className={tab === "transactions" ? "" : "hidden"}>
            <CardHeader
              title="Transactions"
              action={<span className="text-xs text-fg-faint">newest first</span>}
            />
            <CardBody>
          ...
          <Card className={tab === "coins" ? "" : "hidden"}>
            <CardHeader
              title="Coins"
              action={<span className="text-xs text-fg-faint">unspent, newest first</span>}
            />
            <CardBody>
