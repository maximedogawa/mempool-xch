...
export function LearnArticle({ slug, children }: { slug: string; children: ReactNode }) {
  ...
  return (
    <article className="mx-auto flex max-w-3xl flex-col gap-6">
      ...
      <div className="flex flex-col gap-5 text-sm leading-relaxed text-fg-muted [&_h2]:mt-2 [&_h2]:text-base [&_h2]:font-semibold [&_h2]:text-fg [&_strong]:text-fg [&_a]:text-accent [&_a:hover]:underline [&_ul]:list-disc [&_ul]:space-y-1 [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:space-y-1 [&_ol]:pl-5 [&_code]:mono [&_code]:text-xs">{children}</div>
      <nav aria-label="More articles" className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4 text-sm">
        {prev ? (
