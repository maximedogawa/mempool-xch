    ...
    <div className="mx-auto flex max-w-3xl flex-col gap-6">
      <header>
        ...
        <p className="mt-2 text-sm text-fg-muted">
          Short explanations of the things this explorer shows, written for people who use Chia rather than build it. Each article links to the page where you
          can watch the concept live.
        </p>
      </header>
      <ol className="flex flex-col gap-3">
        {LEARN_ARTICLES.map((a, i) => (
          <li key={a.slug}>
            <Link href={learnHref(a.slug)} className="card-lift flex items-start gap-4 rounded-card border border-border bg-surface px-4 py-3 hover:border-border-strong">
              <span className="tabular mt-0.5 text-lg font-semibold text-fg-faint">{String(i + 1).padStart(2, "0")}</span>
