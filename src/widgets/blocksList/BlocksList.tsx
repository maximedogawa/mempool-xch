...
export function BlocksList() {
  ...
  const rows = [...(query.data ?? [])]
    ...
    .filter((r) => !txOnly || r.isTransactionBlock)
    .slice(0, PAGE);
  const totals = useBlocksAssetTotals(
    rows.filter((b) => b.isTransactionBlock).map((b) => ({ height: b.height, hash: b.headerHash }))
  );
  const movedByHeight = new Map(
    rows
      .filter((b) => b.isTransactionBlock)
      .map((b, i) => [
        b.height,
        totals[i]?.data ? formatAmount(BigInt(totals[i]!.data!.xch)) : null,
      ])
  );
      ...
      <CardBody className="flex flex-col gap-3">
                ...
                return (
                  <Tr key={b.height}>
                    ...
                    <Td className="tabular hidden whitespace-nowrap text-fg-muted sm:table-cell">
                      {b.timestamp ? formatAge(b.timestamp * 1000, now) : "—"}
                    </Td>
                    <Td className="tabular hidden text-right text-fg-muted md:table-cell">
                      {b.isTransactionBlock ? (b.rewardClaimsIncorporated?.length ?? 0) : "—"}
                    </Td>
                    <Td className="tabular text-right">
                      {b.isTransactionBlock ? (
                        formatAmount(b.fees ?? 0n)
                      ) : (
                    ...
                    <Td className="tabular hidden text-right text-fg-muted md:table-cell">
                      {b.isTransactionBlock ? (
                        (movedByHeight.get(b.height) ?? <span className="text-fg-faint">…</span>)
                      ) : (
                        <span className="text-fg-faint">—</span>
                      )}
                    </Td>
                    ...
                    <Td
                      className="mono hidden text-xs text-fg-faint xl:table-cell"
                      title={b.headerHash}
                    >
        ...
        <div className="flex items-center justify-between gap-2 text-xs text-fg-faint">
          ...
          <div className="flex gap-2">
            <Button
              size="sm"
              disabled={top === null || peak === null}
              onClick={() =>
                setTop((t) =>
                  t === null || peak === null ? null : t + PAGE >= peak ? null : t + PAGE
                )
              }
            >
              Newer
