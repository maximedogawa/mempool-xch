...
const NO_COINSET = "Coinset's indexed API has no aggregate endpoint for this (verified against its OpenAPI spec); a future provider such as nodexch could add it.";
...
export function ChartsPage() {
  ...
  const spec = (title: string, definition: string, technical: string, formatValue: (v: number) => string): ChartSpec => ({
    title,
    definition,
    technical,
    formatValue,
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Section title="Mempool">
        <ChartCard
          spec={spec("Cost used", "Total CLVM cost of every pending spend bundle.", "Sampled in this browser every time the mempool summary refreshes; kept for 2 hours.", (v) => formatCost(v))}
          points={mempool.available ? mempool.costUsed : null}
          unavailable={mempool.available ? undefined : "Only the last 2 hours are sampled in this browser (Coinset has no mempool history endpoint); pick 6h or 24h to see it."}
          smoothing={controls.smoothing}
          scale={controls.scale}
        />
        <ChartCard
          spec={spec("Waiting bundles", "Spend bundles sitting in the mempool.", "Same 2-hour browser sample as Cost used.", (v) => formatNumber(Math.round(v)))}
          points={mempool.available ? mempool.waitingBundles : null}
          unavailable={mempool.available ? undefined : "Only the last 2 hours are sampled in this browser; pick 6h or 24h to see it."}
          smoothing={controls.smoothing}
          scale={controls.scale}
        />
        <ChartCard
          spec={spec("Total fees", "Fees offered by every pending spend bundle, summed.", "Same 2-hour browser sample as Cost used.", (v) => formatAmount(BigInt(Math.round(v))))}
      ...
      <Section title="Blocks">
        <ChartCard
          spec={spec("Fees per transaction block", "Average total fees paid in a transaction block.", "Averaged per sampling window from get_block_records (block_record.fees); bounded number of windows regardless of range.", (v) => formatAmount(BigInt(Math.max(0, Math.round(v)))))}
        ...
        />
        <ChartCard
          spec={spec("Time between transaction blocks", "Average gap between consecutive transaction blocks.", "Averaged per sampling window from get_block_records timestamps.", (v) => formatDuration(v))}
      ...
      <Section title="Network">
        <ChartCard
          spec={spec("Netspace", "Estimated total space farming the network.", "get_network_space between each sampling window's first and last block — the node's own difficulty-based estimate, not derived by us.", formatBytes)}
          points={network.netspace}
          loading={network.isLoading}
          unavailable={network.netspace.length === 0 && !network.isLoading ? NO_INDEXED("get_network_space did not answer for this endpoint.") : undefined}
          smoothing={controls.smoothing}
          scale={controls.scale}
        />
        <ChartCard
