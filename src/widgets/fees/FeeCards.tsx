  ...
  return (
    <Card>
      <CardHeader
        ...
        action={<Tooltip text={REFERENCE_HINT} />}
      />
      <CardBody className="flex flex-col gap-3">
        <div className="grid grid-cols-3 gap-2">
          {FEE_TARGETS_S.map((target, i) => {
            const estimate = fee.data?.estimates[i];
            const rate = estimate !== undefined ? Number(estimate) / CHIA.REFERENCE_SPEND_COST : null;
            ...
            return (
              ...
              >
                <span className="text-[11px] font-medium uppercase tracking-wider text-fg-muted">{TARGET_LABELS[target]}</span>
                {fee.isLoading ? (
                  <Skeleton className="h-6 w-16" />
                ) : rate !== null ? (
                  <>
                    <span className="tabular text-lg font-semibold leading-tight">
                      {formatFeeRate(rate)} <span className="text-xs font-normal text-fg-muted">mojo/cost</span>
                    </span>
