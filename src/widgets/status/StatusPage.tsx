...
export function StatusPage() {
    ...
    queries: [
      {
        ...
        queryFn: async ({ signal }: { signal: AbortSignal }): Promise<CheckResult> => {
          ...
          return {
            ...
            detail: `${value.synced ? "synced" : "not synced"} · peak #${formatNumber(value.peak.height)}`,
            latencyMs: ms,
          };
        },
      },
        ...
        queryFn: async ({ signal }: { signal: AbortSignal }): Promise<CheckResult> => {
          ...
          return {
            ...
            detail: value.reorgs[0]
              ? `last reorg ${formatAge(value.reorgs[0].detectedAtMs)}`
              : "answering",
            latencyMs: ms,
          };
        },
      ...
      },
      {
        ...
        queryFn: async ({ signal }: { signal: AbortSignal }): Promise<CheckResult> => {
          ...
          return {
            ...
            detail: `${seeder}: ${value.length} nodes`,
            latencyMs: ms,
          };
        },
      },
      {
        ...
        queryFn: async ({ signal }: { signal: AbortSignal }): Promise<CheckResult> => {
          ...
          return {
            ...
            detail: value.get("8.8.8.8") ? "geolocation answering" : "answered without a location",
            latencyMs: ms,
          };
        },
      },
  ...
  const rows: {
    ...
    {
      name: endpoints.isCoinset ? "Coinset full-node RPC" : "Your node (full-node RPC)",
      what: endpoints.rpcUrl.replace(/^https?:\/\//, ""),
      index: 0,
    },
    {
      name: "Coinset indexed API",
      what: endpoints.indexedUrl?.replace(/^https?:\/\//, "") ?? "not available on a custom node",
      index: 1,
    ...
    r.live
      ...
      : r.index !== undefined
        ? checks[r.index]?.error
          ? ({
              health: "down",
              detail: errorMessage(checks[r.index]!.error),
              latencyMs: 0,
            } as CheckResult)
          : checks[r.index]?.data
        : undefined
    ...
    <div className="flex flex-col gap-5">
      <header className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          ...
          <Tooltip
            text="Measured from your browser right now, the same way the pages fetch their data. There is no server-side monitor and no history; a red row means your connection cannot reach that service at the moment."
            placement="bottom"
          />
        </div>
      </header>
        ...
        <div className="flex items-center gap-2 text-sm font-semibold">
          ...
          <span data-testid="status-overall">
            {worst === "ok"
              ...
              : worst === "degraded"
                ? "Some services are slow or degraded"
                : "Some services are unreachable"}
          </span>
        </div>
        <div className="flex items-center gap-3 text-xs text-fg-faint">
          {checkedAt > 0 ? (
            <span>checked {formatAge(checkedAt, now)} · re-checks every minute</span>
          ) : (
            <span>checking…</span>
          )}
          <Button
                ...
                >
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    <span className="hidden text-fg-muted sm:inline">
                      {result?.detail ?? (pending ? "checking…" : "")}
                    </span>
                    ...
                    <span
                      className={cn(
                        ...
                        tone ? tone.className : "bg-surface-2 text-fg-muted"
                      )}
                      data-testid={`status-${row.name.toLowerCase().replace(/[^a-z]+/g, "-")}`}
                    >
      ...
      <p className="text-xs text-fg-faint">
        The site itself is a static app served from mempoolxch.space (health endpoint{" "}
        <span className="mono">/up</span>); everything else is fetched by your browser from the
        services above. Coinset publishes its own status independently of this page.
      </p>
    </div>
  );
}
