...
import { launcherIdToNftId, nftIdToLauncherId, puzzleHashToAddress } from "@/shared/lib/chia/address";
...
export function NftPage() {
  ...
  const history = usePagedTransactions({
    queryKey: useCallback((cursor: string | null) => queryKeys.nft(network, launcherId, "history", cursor), [network, launcherId]),
    fetchPage: useCallback((cursor: string | null, limit: number, signal: AbortSignal) => client.getTransactionsByNftId(launcherId, { cursor: cursor ?? undefined, limit }, signal), [client, launcherId]),
    enabled: ids !== null && client.hasIndexed,
  });

  if (!ids) {
    return <EmptyState tone="danger" title="Not a valid NFT id" description={`Expected an nft1… id or a 32-byte launcher id. Got: ${raw || "(empty)"}`} />;
  }

  const latestRecord = latest.data;
  const ownerP2 = (typeof latestRecord?.p2 === "string" ? stripHexPrefix(latestRecord.p2) : null) ?? metadata.data?.ownerP2 ?? null;
  ...
  const royaltyBps = typeof royalty.royalty_basis_points === "number" ? royalty.royalty_basis_points : metadata.data?.royaltyBasisPoints;
  const name = metadata.data?.name ?? (metadata.isLoading ? null : `NFT ${ids.nftId.slice(0, 12)}…`);
    ...
    <div className="flex flex-col gap-5">
      <Card>
        ...
        <CardBody className="grid grid-cols-1 gap-5 md:grid-cols-[280px_minmax(0,1fr)]">
          <AssetImage urls={metadata.data?.imageUrls ?? []} alt={name ?? "NFT image"} className="aspect-square w-full max-w-[280px] justify-self-center md:justify-self-start" />
          <div className="flex min-w-0 flex-col gap-3">
            <div>
              <h1 className="text-2xl font-semibold">{name ?? <Skeleton className="h-7 w-48" />}</h1>
              {metadata.data?.collectionName ? <p className="text-sm text-fg-muted">Collection: {metadata.data.collectionName}</p> : null}
              {metadata.data?.description ? <p className="mt-1 line-clamp-4 text-sm text-fg-faint [overflow-wrap:anywhere]">{metadata.data.description}</p> : null}
              {!metadata.isLoading && !metadata.data && network === "mainnet" ? <p className="text-xs text-fg-faint">Metadata not available from MintGarden.</p> : null}
              {network !== "mainnet" ? <p className="text-xs text-fg-faint">NFT metadata lookup is mainnet only.</p> : null}
            </div>
            <dl className="grid grid-cols-1 gap-3 text-sm">
              <div className="min-w-0">
                <dt className="text-[11px] font-medium uppercase tracking-wider text-fg-muted">NFT id</dt>
              ...
              <div className="min-w-0">
                <dt className="text-[11px] font-medium uppercase tracking-wider text-fg-muted">Launcher id</dt>
              ...
              <div className="min-w-0">
                <dt className="text-[11px] font-medium uppercase tracking-wider text-fg-muted">Current owner</dt>
                <dd className="mono break-all">
                  {ownerAddress ? (
                    <Link href={routes.address(ownerAddress)} className="text-accent hover:underline">
                      {ownerAddress}
                    </Link>
                  ) : latest.isLoading || metadata.isLoading ? (
                    <Skeleton className="h-5 w-64" />
                  ) : (
                    <span className="text-fg-faint">unknown{!client.hasIndexed ? " (needs Coinset)" : ""}</span>
                  )}
                </dd>
              </div>
            ...
            {network === "mainnet" ? (
              <a href={`https://mintgarden.io/nfts/${ids.nftId}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm text-accent hover:underline">
      ...
      <Card>
        ...
        <CardBody>
          {client.hasIndexed ? (
            <TxSummaryList transactions={history.transactions} loading={history.isLoading} error={history.error} emptyText="No transfers indexed for this NFT." hasMore={history.hasMore} onLoadMore={history.loadMore} loadingMore={history.loadingMore} />
          ) : (
