...
function CoinRow({ coin }: { coin: FlowCoin }) {
  ...
  return (
    <li className="flex flex-col gap-1 rounded-sm border border-border bg-bg px-3 py-2">
    ...
    <div className="flex min-w-0 flex-1 flex-col gap-2">
      ...
      {coins.length === 0 ? (
        ...
        <ul className="flex flex-col gap-1.5">
          {shown.map((c, i) => (
            <CoinRow key={`${c.coinId || c.puzzleHash}-${i}`} coin={c} />
          ))}
        </ul>
      )}
