...
function Unavailable({ what }: { what: string }) {
  return (
    <p className="rounded-sm border border-warning/40 bg-[color-mix(in_srgb,var(--warning)_10%,transparent)] px-3 py-2 text-xs text-fg-muted">
      <span className="font-semibold text-warning">Unavailable with a custom node:</span> {what} need the Coinset indexed API. Switch the endpoint back to Coinset in{" "}
...
export function AddressPage() {
  ...
  const resolved = useMemo(() => resolveAddressId(raw, networkConfig.addressPrefix), [raw, networkConfig.addressPrefix]);
  ...
  return (
    <div className="flex flex-col gap-5">
        ...
        <CardBody className="flex flex-col gap-4 md:flex-row md:items-start">
          ...
          <dl className="grid min-w-0 flex-1 grid-cols-1 gap-x-6 gap-y-3 text-sm">
            <div className="min-w-0">
              <dt className="text-[11px] font-medium uppercase tracking-wider text-fg-muted">{isDid ? "DID id" : "Address"}</dt>
            ...
            <div className="min-w-0">
              <dt className="text-[11px] font-medium uppercase tracking-wider text-fg-muted">{isDid ? "Launcher id" : "Puzzle hash"}</dt>
            ...
            {!isDid ? (
              <div className="min-w-0">
                <dt className="text-[11px] font-medium uppercase tracking-wider text-fg-muted">Other network prefix</dt>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatTile
          label="XCH balance"
          value={xchBalance !== undefined ? formatAmount(xchBalance) : data.xch.isLoading || data.coins.isLoading ? <Skeleton className="h-6 w-24" /> : "n/a"}
          sub={data.indexed && data.xch.data ? (pendingDelta !== 0n ? `pending ${pendingDelta > 0n ? "+" : "−"}${formatAmount(pendingDelta < 0n ? -pendingDelta : pendingDelta)}` : "no pending change") : "from unspent coins"}
          tone="primary"
        />
        <StatTile label="CAT balances" value={data.indexed ? (data.cats.data ? formatNumber(data.cats.data.filter((c) => c.confirmed > 0n).length) : "…") : "n/a"} sub={data.indexed ? "tokens held" : "needs Coinset"} />
        <StatTile label="NFTs" value={data.indexed ? (data.nfts.data !== undefined ? formatNumber(data.nfts.data) : "…") : "n/a"} sub={data.indexed ? "owned" : "needs Coinset"} />
        <StatTile label="Unspent coins" value={unspentCount !== null ? formatNumber(unspentCount) : "…"} sub={coins ? `${formatNumber(coins.xchCoins.length)} XCH · ${coins.hintedCoins ? `${formatNumber(coins.hintedCoins.length)} hinted` : "hinted n/a"}` : undefined} hint="Coins locked to this puzzle hash plus CAT, NFT and DID coins hinted to it." />
      </div>

      {!data.indexed ? <Unavailable what="Balances by asset, NFT count, pending and confirmed transaction history" /> : null}

      {data.indexed && data.cats.data && data.cats.data.some((c) => c.confirmed > 0n || c.pending !== 0n) ? (
        <Card>
                ...
                {data.cats.data
                  ...
                  .map((c) => {
                    ...
                    return (
                      <Tr key={c.assetId}>
                        <Td>
                          <Link href={routes.cat(c.assetId)} className="flex items-center gap-2 hover:underline md:whitespace-nowrap">
                            <AssetImage urls={token?.iconUrl ? [token.iconUrl] : []} alt={token?.name ?? "token"} className="h-6 w-6 shrink-0" rounded="rounded-full" />
                        ...
                        <Td className="tabular text-right text-fg-muted">{c.pending === 0n ? "—" : formatCat(c.pending)}</Td>
                      </Tr>
                    );
                  })}
              </tbody>
      ...
      {data.indexed ? (
        <Card>
          ...
          <CardBody>
            <TxSummaryList transactions={data.pending.transactions} loading={data.pending.isLoading} error={data.pending.error} viewedP2={ph} tokens={tokens.data} emptyText="No pending transactions for this address." hasMore={data.pending.hasMore} onLoadMore={data.pending.loadMore} loadingMore={data.pending.loadingMore} />
          </CardBody>
        </Card>
      ) : null}
      ...
      <Card>
        ...
        <CardBody>
          {data.indexed ? (
            <TxSummaryList transactions={data.history.transactions} loading={data.history.isLoading} error={data.history.error} viewedP2={ph} tokens={tokens.data} emptyText="No confirmed transactions found for this address." hasMore={data.history.hasMore} onLoadMore={data.history.loadMore} loadingMore={data.history.loadingMore} />
          ) : (
...
function CoinList({ coins, loading }: { coins: CoinFallback | undefined; loading: boolean }) {
  ...
  const all = coins ? [...coins.xchCoins.map((c) => ({ c, hinted: false })), ...(coins.hintedCoins ?? []).map((c) => ({ c, hinted: true }))] : [];
      ...
      <tbody>
        {all.slice(0, 200).map(({ c, hinted }) => (
          <Tr key={c.name}>
            ...
            <Td>{hinted ? <Badge tone="cat">hinted asset</Badge> : <Badge tone="xch">XCH</Badge>}</Td>
            <Td className="tabular text-right">{hinted ? `${formatNumber(Number(c.coin.amount))} mojo` : formatAmount(c.coin.amount)}</Td>
