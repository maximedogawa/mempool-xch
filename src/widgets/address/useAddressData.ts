...
export function useAddressData(p2: string | null) {
    ...
    queryFn: async ({ signal }): Promise<CoinFallback> => {
      const [xchCoins, hintedCoins] = await Promise.all([
        ...
        client
          ...
          .catch(() =>
            new Promise<null>((resolve) => setTimeout(() => resolve(null), 1_500)).then(() =>
              client.getCoinRecordsByHint(ph, false, signal)
            )
          )
          .catch(() => null),
      ]);
      return {
        xchCoins,
        hintedCoins,
        xchBalance: xchCoins.reduce((s, c) => s + c.coin.amount, 0n),
      };
    },
    enabled,
    staleTime: 30_000,
  ...
  const pending = usePagedTransactions({
    queryKey: useCallback(
      (cursor: string | null) => queryKeys.address(network, ph, "pending", cursor),
      [network, ph]
    ),
    fetchPage: useCallback(
      (cursor: string | null, limit: number, signal: AbortSignal) =>
        client.getPendingTransactionsByP2(ph, { cursor: cursor ?? undefined, limit }, signal),
      [client, ph]
    ),
    enabled: indexed,
  ...
  const history = usePagedTransactions({
    queryKey: useCallback(
      (cursor: string | null) => queryKeys.address(network, ph, "history", cursor),
      [network, ph]
    ),
    fetchPage: useCallback(
      (cursor: string | null, limit: number, signal: AbortSignal) =>
        client.getTransactionsByP2(ph, { cursor: cursor ?? undefined, limit }, signal),
      [client, ph]
    ),
    enabled: indexed,
  });
