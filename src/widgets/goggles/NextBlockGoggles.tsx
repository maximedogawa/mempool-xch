...
export function NextBlockGoggles() {
  ...
  const cells = useMemo(
    () =>
      next
        ? squarify(
            next.items.map((item) => ({ item, weight: item.cost })),
            W,
            fillHeight
          )
        : [],
  ...
  const perKind = useMemo(() => {
    const acc: Partial<Record<KindFilter, { count: number; cost: number }>> = {
      all: { count: next?.items.length ?? 0, cost: next?.totalCost ?? 0 },
    };
  ...
  const isFresh = (item: CompactMempoolItem) =>
    baseline.current !== null &&
    item.firstSeen > baseline.current &&
    now - item.firstSeen < FRESH_MS;
  ...
  const matches = (item: CompactMempoolItem) =>
    ...
    (fee === "any" || feeBandFor(item.feeRate).id === fee) &&
    (only === "all" || (only === "new" ? isFresh(item) : mine.has(item.id)));
          ...
          <>
            ...
            <div className="relative">
              <svg
                ...
                role="group"
                aria-label={label}
                className="block h-auto w-full overflow-hidden rounded-card bg-bg"
              >
                <title>{label}</title>
                <defs>
                  <pattern
                ...
                {cells.map((cell, cellIndex) => {
                  ...
                  const token =
                    primary.kind === "cat" && primary.assetId
                      ? tokens.data?.[primary.assetId]
                      : undefined;
                  const amountText = formatPrimaryAsset(primary, token?.symbol);
                  const image =
                    item.kind === "nft" && item.assetIds[0] ? nftImage.get(item.assetIds[0]) : null;
                    ...
                    >
                      <g
                        ...
                        style={{
                          ...
                          animationDelay: `${Math.min(cellIndex, 40) * 12}ms`,
                        }}
                      >
                        <rect
                          {...inner}
                          rx={3}
                          fill={fill}
                          fillOpacity={mode === "fee" ? 0.88 : 0.6}
                          stroke={KIND_COLOR[item.kind]}
                          strokeWidth={item.kind === "xch" ? 0 : 1.5}
                        />
                        {image && big ? (
                          <>
                            ...
                            <image
                              ...
                              clipPath={`url(#clip-${item.id.slice(0, 12)})`}
                              opacity={0.9}
                            />
                          </>
                        ) : null}
                        ...
                        {big ? (
                          <foreignObject
                            x={cx + 4}
                            y={cy + 4}
                            width={Math.max(0, cell.width - 8)}
                            height={Math.max(0, cell.height - 8)}
                            className="pointer-events-none"
                          >
                          ...
                          <text
                            ...
                            className="pointer-events-none"
                          >
                            {amountText.length > Math.floor(cell.width / 6.2)
                              ? `${amountText.slice(0, Math.max(3, Math.floor(cell.width / 6.2) - 1))}…`
                              : amountText}
                          </text>
                        ) : null}
                      </g>
