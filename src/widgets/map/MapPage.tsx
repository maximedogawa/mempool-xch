...
export function MapPage() {
  ...
  const kindOf = useMemo(
    () => new Map(summary.data?.items.map((i) => [i.id, i.kind]) ?? []),
    [summary.data]
  );
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
        ...
        <StatTile
          ...
          sub={countries[0] ? `${countries[0].country} leads` : undefined}
        />
        <StatTile
          label="IPv6"
      ...
      <Card>
            ...
            <span className="flex flex-wrap items-center gap-3 text-[11px] text-fg-faint">
              ...
              {!endpoints.isCoinset ? (
                <span className="inline-flex items-center gap-1">
                  <span
                    aria-hidden="true"
                    className="inline-block h-2.5 w-2.5 rounded-full border-2 border-kind-offer"
                  />{" "}
                  connected peer
                </span>
              ) : null}
              <span className="inline-flex items-center gap-1">
                <span
                  aria-hidden="true"
                  className="inline-block h-2.5 w-2.5 rounded-full border border-warning"
                />{" "}
                block ·{" "}
                <span
                  aria-hidden="true"
                  className="inline-block h-2.5 w-2.5 rounded-full border border-primary"
                />{" "}
                bundle (modelled)
              </span>
            </span>
        ...
        <CardBody className="relative">
          ...
          <div
            role="status"
            className="pointer-events-none absolute left-4 top-4 rounded-sm border border-border bg-bg-elevated/95 px-2.5 py-1.5 text-xs shadow-card"
            style={{ visibility: hoveredCluster ? "visible" : "hidden" }}
          >
          ...
          <CardBody>
                ...
                <tbody>
                  {countries.slice(0, COUNTRY_ROWS).map((c) => (
                    <Tr key={c.countryCode}>
                      <Td>
                        <span className="mr-1.5 text-[10px] font-semibold uppercase text-fg-faint">
                          {c.countryCode}
                        </span>
                        {c.country}
                      </Td>
            ...
            {countries.length > COUNTRY_ROWS ? (
              <p className="mt-2 text-xs text-fg-faint">
                and {formatNumber(countries.length - COUNTRY_ROWS)} more countries.
              </p>
            ) : null}
          </CardBody>
        </Card>
          ...
          <CardHeader
            ...
            action={
              <span className="tabular text-xs text-fg-faint">
                ...
                {formatNumber(activity.counts.bundles)} bundles since opening
              </span>
            }
          />
          <CardBody className="flex flex-col gap-3">
            {activity.feed.length === 0 ? (
              <p className="py-4 text-center text-sm text-fg-faint">
                Waiting for the first event from the live stream…
              </p>
            ) : (
                ...
                {activity.feed.map((e) => (
                  ...
                  >
                    {e.kind === "block" ? (
                      <span>
                        <span
                          className="mr-1.5 inline-block h-2 w-2 rounded-full bg-warning"
                          aria-hidden="true"
                        />
                        New peak{" "}
                        <Link
                      ...
                      <span className="flex flex-wrap items-center gap-1.5">
                        ...
                        {e
                          .ids!.map((id) => kindOf.get(id))
                          .filter((k): k is NonNullable<typeof k> => !!k && k !== "xch")
                          .slice(0, 2)
                          .map((k, i) => (
                            <KindBadge key={`${k}-${i}`} kind={k} />
                          ))}
                      </span>
                    )}
